...
# Plot MULTIPOLYGON and POINT data on the same map
tm_shape(multipolygon_data) +
  tm_polygons(alpha = 0.5, col = "blue") +
...
```{r}
# Rename the common column in geo_data to "id" to match mc2_nodes_filtered
geo_data <- geo_data %>%
  rename(id = Name)  # Adjust "name" to the actual column name that matches
...
```{r}
# Aggregating port exits by time intervals (e.g., days)
exit_counts <- table(cut(mc2_links_prep$time, breaks = "day"))

# Convert to data frame
exit_counts_df <- as.data.frame(exit_counts)
exit_counts_df$Date <- as.Date(levels(exit_counts_df$Var1)[exit_counts_df$Var1])

# Plotting seasonal trends
ggplot(exit_counts_df, aes(x = Date, y = Freq)) +
  geom_line() +
  labs(x = "Date", y = "Port Exit Count", title = "Seasonal Trends in Port Exit Records") +
  theme_minimal()
