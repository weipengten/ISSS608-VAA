...
transponder_ping_df2 <- transponder_pings %>%
  inner_join(mc2_nodes %>% select(id, company), by = c("vessel" = "id")) %>% 
  mutate(date =ping_date,
         start_time = time)

# Define the date range for filtering
start_date <- as.Date('2035-04-27')
end_date <- as.Date('2035-05-27')

transponder_ping_south_df2 <- transponder_ping_df2 %>% 
  filter(vessel == "roachrobberdb6" ,
         between(date, start_date, end_date))
  
data <- transponder_ping_south_df2 %>% 
  arrange(start_time) %>% 
...
ggplot(data, aes(x = date, y = location_num, group = vessel)) +
  geom_point(aes(color = location_factor), size = 4) + 
  geom_path(aes(color = location_factor), size = 1, arrow = arrow(type = "open", length = unit(0.02, "npc"))) + 
  facet_wrap(~ vessel, ncol = 1, scales = "free_y") + 
  scale_y_continuous(breaks = unique(data$location_num), labels = unique(data$location)) + 
  labs(title = "Path of Vessel Locations Over Time", 
       x = "Time", 
       y = "Location", 
       color = "Location") + 
  theme_minimal() + 
...
```{r}
# Arrange data by start_time and calculate end_time
data <- transponder_ping_south_df2 %>% 
  arrange(start_time) %>% 
  mutate(end_time = lead(start_time)) 
...
ggplot() +
  geom_path(data = mc2_nodes_geo_sf, aes(x = Longitude, y = Latitude, group = location), color = "blue") +
  geom_point(data = mc2_nodes_geo_sf, aes(x = Longitude, y = Latitude), color = "red") +  # Plot nodes
